function Process2MClustLightOnly(expdate, session, filenum, varargin)
%specialized version of Process2MClust that only extracts waveforms during
%AO pulses, so we can focus on clustering only cells that spike during
%light-on pulses.
%
%
...
%get samprate from experfile
samprate= getsamplerate(expdate,session,filenum); %in Hz

% Find samples for which light was on
samples_on=[];
for e=1:length(event)
        switch event(e).Type
        case 'aopulse'
            dur=event(e).Param.duration; % in ms
            pos=event(e).Position; %in samples
            samples_on=[samples_on pos:pos+samprate*dur/1000];
    end
end
...
%keep only spikes during light on (samples_on)
allspikes_orig=allspikes; %sanity check
s=ismember(allspikes, samples_on);
...
allspikes=allspikes(allspikes>9);
allspikes=allspikes(allspikes<length(scaledtrace1)-22);

%         remove spikes occurring within retrigger_time (handling same as
%         refractory period above)
dallspikes=allspikes(1+find(diff(allspikes)>retrigger_time));
dallspikes=[allspikes(1) dallspikes(:)'];

t=1000*allspikes/samprate;

wf(:,1,:)=window(scaledtrace1, allspikes-9, 32);
wf(:,2,:)=window(scaledtrace2, allspikes-9, 32);
wf(:,3,:)=window(scaledtrace3, allspikes-9, 32);
wf(:,4,:)=window(scaledtrace4, allspikes-9, 32);
...
function out = window(x,st,n)
%extracts windows of length n from vector x at points st
a = repmat((1:n)',1,length(st));
b=repmat(st,n,1);
c=a+b;
out = x(c)';

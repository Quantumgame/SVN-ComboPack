function PlotTC_psth_PrePost(expdate1, session1, filenum1, expdate12, session2, filenum2, varargin)
% extracts spikes and plots two single-channel psth tuning curves (a pre and a post) 
% usage: PlotTC_psth_PrePost(expdate1, session1, filenum1, expdate12, session2, filenum2, [thresh], [xlimits], [ylimits], [binwidth], [chan])
% [thresh], [xlimits], [ylimits], [binwidth], [chan] are optional)
%
% chan defaults to 1; for dual electrode experiments, set to either 1 or 2
% E2 analysis function
% mw 092702
% last update mw 05162011 - now plots mean spike rate (in Hz) averaged across trials
...
elseif nargin==10
    ...
    if isempty(ylimits) ylimits=-1;end
    binwidth= varargin{4};
    if isempty(binwidth) binwidth=10;end
    chan=1;
elseif nargin==11
    nstd=varargin{1};
    if isempty(nstd) nstd=3;end
...
datafile2=sprintf('%s-%s-%s-%s-AxopatchData%d-trace.mat', expdate12, username, session2, filenum2, chan);
eventsfile2=sprintf('%s-%s-%s-%s-AxopatchData%d-events.mat', expdate12, username, session2, filenum2, chan);
...
E=load(eventsfile1);
    ...
    godatadirbak(expdate1, session1, filenum2)
    fprintf('\nload file 1: ')
fprintf('\ntrying to load %s...', datafile1)
...
godatadir(expdate12, session2, filenum2)
L=load(datafile2);
E=load(eventsfile2);
catch
fprintf('\nload file 2: ')
fprintf('\ntrying to load %s...', datafile2)
godatadirbak(expdate12, session2, filenum2)
L=load(datafile2);
E=load(eventsfile2);
end
fprintf('done.');
...
filteredtrace2=filtfilt(b,a,scaledtrace2);
    ...
    fprintf('\nboth file 1 & 2: using spike detection threshold of %.1f mV (%d sd)', thresh1, nstd);
...
spikes2=find(abs(filteredtrace2)>thresh1);
...
%                 dur=event2(i).Param.duration;
                ...
                M2(findex,aindex,dindex, nreps2(findex, aindex, dindex)).spiketimes=spiketimes2;
            end
        end
    end
end
...
if ylimits==-1
    ...
    for aindex=[numamps:-1:1]
        for findex=1:numfreqs
            ...
            n1=n1./nreps1(findex, aindex, dindex); %normalize to spike rate (averaged across trials)
            n1=1000*n1./binwidth; %normalize to spike rate in Hz
            n2=n2./nreps2(findex, aindex, dindex); %normalize to spike rate (averaged across trials)
            n2=1000*n2./binwidth; %normalize to spike rate in Hz
...
title(sprintf('%s-%s-%s, %s-%s-%s, ch%d, nstd=%d, %dms bins, %d,%d total spikes',expdate1,session1, filenum1,expdate12,session2, filenum2, chan, nstd,binwidth,length(dspikes1), length(dspikes2)))

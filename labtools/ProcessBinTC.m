function ProcessBinTC(varargin)
%
%usage: ProcessBinTC(expdate, session, filename)
%ProcessBinTC(expdate, session, filename, xlimits)
%ProcessBinTC(expdate, session, filename, xlimits, ylimits)
%
%processes a Vm or LFP binaural tuning curve and saves to vm outfile (outvm)
%
...
out.xlimits=xlimits;
out.ylimits=ylimits;
outfile=sprintf('outvm%s-%s-%s.mat', expdate, session, filenum);
godatadir(expdate, session, filenum)
save (outfile, 'out')
fprintf('\n saved to %s', outfile)

function p=get_correctedV_NBN(p)
    ...
    for findex=1:length(p.freqs)
        for aindex=1:length(p.amps)
            for pindex=1:length(p.potentials)
                ...
                p.correctedV(findex, aindex, bwindex, pindex, :)=p.corrected_potentials(pindex)-p.meanrs*mI(pindex,:)/1000;

 function MakeSpNoiseProtocol(varargin)
...
for nr=1:nreps
    for ne=1:nepochs
        ...
        sample.sample=ss(ne,:);
        env=Env(ne,:);
        sourcefilename=sprintf('SPNoise_sourcefile_%ddB_%.1fs_%db%s_isi%dms_%de_n%d.mat', amp, duration, nbands,bandstr, 1000*isi, ne, nreps);
        envfilename=sprintf('SPNoise_envfile_%ddB_%.1fs_%db%s_isi%dms_%de_n%d.mat', amp, duration, nbands,bandstr, 1000*isi, ne, nreps);

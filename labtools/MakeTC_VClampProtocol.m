function MakeTC_VClampProtocol(isi, pulse_height, pulse_width, npulses)
...
% usage: MakeTC_VClampProtocol(isi, pulse_height, pulse_width, npulses)
% inputs:
%   isi   -  delay to the start of the pulse after the trigger (ms), and
%       in between pulse and tones
%   pulse_height: describes current pulse, mV
...
%
%example call:
% MakeTC_VClampProtocol(500, -10, 100, 10)
% 
% 
...
stimuli(1).param.name= sprintf('TC_VClamp, isi:%d/ph:%d/pw:%d/np:%d/%s', isi, pulse_height, pulse_width,npulses, tc.stimuli(1).param.name);
stimuli(1).param.description=sprintf('TC_VClamp, isi: %dms, pulse_height: %dmV, pulse_width: %d ms, %s',isi,pulse_height, pulse_width,tc.stimuli(1).param.description);
filename=sprintf('TC_VClamp-%dms-%dmV-%dms-%s', isi, pulse_height, pulse_width, tcfilename);

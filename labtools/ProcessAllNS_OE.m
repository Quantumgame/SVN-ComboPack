function ProcessAllNS_OE(varargin)
%usage: ProcessAllNS_OE(expdate, session, filename, [xlimits], [ylimits], [channel])
...
soundfilenames=(unique(allfilenames));
dur=max(alldurs); %phonemes will have different durs so just use max dur
isi=unique(allisis);
...
for i=1:length(event)
    if strcmp(event(i).Type, 'naturalsound')
        ...
        stop=(pos/samprate+xlimits(2)*1e-3);
        ...
        if isempty(find(region<0)) %(disallow negative start times)
            ...
            soundnum=find(strcmp(soundfilenames, soundfile));
...
for clust=1:Nclusters
    for soundid=1:length(soundfilenames)
        spiketimes1=[];
        for rep=1:nreps(soundid)
            spiketimes1=[M1(clust, soundid, rep).spiketimes spiketimes1 ];
        end
        mM1(clust, soundid).spiketimes=spiketimes1;
        mM1stim(clust, soundid,:)=mean(M1stim(clust, soundid, 1:nreps(soundid),:), 3);
    end
end
...
out.soundfilenames=soundfilenames;

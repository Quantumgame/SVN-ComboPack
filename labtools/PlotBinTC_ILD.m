function PlotBinTC_ILD(expdate,session,filenum,varargin)
...
%
% usage: PlotBinTC_ILD('expdate','session','filenum',[thresh],[xlimits],[ylimits],binwidth,monitor)


% created using PlotBinTC_psth
% mak 3Aug2012
% latest updates:
...
if j~=length(event)
    warning('ensure allRamps/Lamps/durs/freqs don''t need to be trimmed') %#ok
    % mak 31July2012, this may form a bug when using multiple hold commands
end
freqs=unique(allfreqs(1:j));
...
nrepsmax=max(unique(nreps));
nrepsmin=min(unique(nreps));
monoflag=0;
if max(unique(nreps(:,2:end,2:end,:)))==0  
    monoflag=1;
end
if length(unique(nreps))~=2
...
%plot ILD/ABL/mono BinTC_ILD
if ~monoflag
    ILDs=-40:10:40;
    ABLs=fliplr(max(Ramps)-20:-5:15);
    MonoAmps=Ramps(find(nreps(1,:,1)~=0)); %same for either L or R
...
%                 freq='WN';
%             end
%             title(sprintf('%s-%s-%s-%s, %s (%d ms)\n',expdate,username,session,filenum,freq,durs(dindex)))
        end
    end
end

function Plot_Mouse_NBPPI(expdate1,session1,filenum1 )

%Similar to the rat "PlotASRMike1(expdate,session,filenum )"

%Plot_Mouse_NBPPI('MMDDYY','DIR','FIL')
...
fprintf('\n\nMouse: #__________ Genotype:_____________________ Age:__________ Sex:_____'),
figure;hold on
p=0;
...
plot(numprepulseamps+1, peak_prepulse(:), 'ro')
plot(numprepulseamps+2, peak_baseline(:), 'mo')
plot(numprepulseamps+1, mean(peak_prepulse(:)), 'ko')
plot(numprepulseamps+2, mean(peak_baseline(:)), 'ko')
...
text(numprepulseamps+1, -50, 'PP')
text(numprepulseamps+2, -50, 'BL')

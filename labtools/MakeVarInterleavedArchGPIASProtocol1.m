function MakeVarInterleavedArchGPIASProtocol1
% usage: MakeVarInterleavedArchGPIASProtocol1
%Create interleaved VarPPALaser Protocol and incorporate a previously
% created GPIAS protocol
%this is version 1 which interleaves a laser pulse on every other GPIAS
%(identical repetition of the GPIAS), the laser is on exactly during the
%gap. I.e. laser onset coincides with gap onset, laser duration matches gap
%duration
...
while tc_n+1<=length(tc.stimuli)
    ...
    stimuli(st_n).param.VarPPApulsewidth=tone.param.gapdur; %for example
...
stimuli(1).param.name= sprintf('VarPPALaser GPIAS1, %s', tc.stimuli(1).param.name);
str='laser is on exactly during the gap. I.e. laser onset coincides with gap onset, laser duration matches gap duration';
stimuli(1).param.description=sprintf('VarPPALaser, GPIAS1 (%s) %s', str, tc.stimuli(1).param.description);
filename=sprintf('VarPPALaser GPIAS1-%s', tcfilename);

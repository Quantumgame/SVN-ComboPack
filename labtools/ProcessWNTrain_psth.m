function ProcessWNTrain_psth(expdate, session, filenum, varargin)
...
%durations from WNTrain events files are kind of wacky, I'm setting 
%dfault xlimits based on max isi
if isempty(xlimits)
    xlimits=[-100 max(200, 1.5*max(isis))];
end
...
%the idea here is that for p2/p1 stimuli (only 2 clicks, WNTrain1-style) we
%have an nclicks for each isi.
%if you want to know if it's WNTrain1 or 2, check whether
%length(unique(nclicks))==1
if numnclicks==1
    nclicks=repmat(nclicks, 1, numisis);
end
...
% % mean across reps
p1=Mspikecount(:,1,:); %summed across all isis (since all p1s are identical)
p1=p1(:);

p2=squeeze(Mspikecount(:,2,:));
%./numisis*nreps
if size(p2,1)==1 | size(p2,2)==1
    %vector, only 1 isi
mP2P1=(sum(p2)./nreps')./(sum(p1)./(numisis*nreps'));
else %matrix, multiple isis
mP2P1=(sum(p2, 2)./nreps')./(sum(p1)./(numisis*nreps'));
end

% should probably assess facilitation/suppression using wilcox ranksum
% which is like ttest2 (unpaired samples), but non-parametric
%ttest2 is more attractive here because I can do it sidedly to indpendently
%assess facil and suppres
for isiindex=[1:numisis]
[prs(isiindex),hrs(isiindex)]=ranksum(p2(isiindex,:), p1);
[htf(isiindex), ptf(isiindex)]=ttest2(p2(isiindex,:), p1, [],'right');
[hts(isiindex), pts(isiindex)]=ttest2(p2(isiindex,:), p1, [],'left');
end

%assign outputs
out.facilitation_h=htf;
out.suppression_h=hts;
out.facilitation_p=ptf;
out.suppression_p=pts;

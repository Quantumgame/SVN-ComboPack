function PlotRLF_timecourse_many_LFP(varargin)
%plots time course of LFP responses for each tone frequency and level
...
%you must run PlotRLF_timecourse_LFP on each file first. Specify xlimits
%at the time you run PlotRLF_timecourse_LFP.
%
%usage:
%   PlotRLF_timecourse_many_LFP(expdate1, session1, filenum1, expdate2, session2, filenum2, ... , [stepsize])
...
%
%example call: PlotRLF_timecourse_many_LFP('080409','001','001','080409','001','002','080409','001','003')
if nargin==0
    fprintf('\nno input\n')
    return
elseif nargin<3
...
for filen=1:nfiles
    ...
    user=whome;
    outfiledir=sprintf('d:\\lab\\Data-%s-processed\\%s-%s\\%s-%s-%s',user,expdate(filen,:),user,expdate(filen,:),user,session(filen,:));
    cd(outfiledir);
...
xlabel('time (minutes)')
ylabel('mean spike count (+/- sem)')
title(sprintf('RLF timecourse for %s-%s-%s (%d files), thresh=%g', expdate(1,:), session(1,:), filenum(1,:), nfiles, Thresh(1)))
for filen=1:nfiles
    yl=ylim;
    text(xpos(filen), yl(1)+.9*diff(yl), (filenum(filen,:)))
end

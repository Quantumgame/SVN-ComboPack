function p=compute_ge_gi(p, varargin)
% usage: 
    ...
    for findex=1:numfreqs
        for aindex=1:numamps
            ...
            for pindex=1:numpotentials
                is(:,pindex)=squeeze(p.mM1(findex,aindex, dindex, pindex,1, :));
                vs(:,pindex)=squeeze(p.correctedV(findex,aindex, dindex, pindex, :));
            end
...
for findex=1:numfreqs
    for dindex=1:numdurs
        for aindex=1:numamps
            ...
            fprintf('\nf%d a%d d%d', findex, aindex, dindex);
            for pindex=1:numpotentials
                spoo= pindex;
                ...
                rawI(:,:,spoo)=squeeze(p.M1(findex, aindex, dindex, pindex,1:min_num_reps, :)); %use all currents
                ...
                try rawdeltaI(:,:,spoo) = rawI(:,:,spoo) - repmat(mean(rawI(:,1:baseline,spoo), 2), 1, datalength);
                catch
                    % there was a problem with ira's long data files,
                    % possibly when using lostat. Let's see if this catch
                    % helps mk 12sep2013
                    rawdeltaI(:,:,spoo) = rawI(:,:,spoo) - repmat(mean(rawI(:,1:datalength,spoo), 2), 1, datalength);
                end
...
%                 workaround using default xlimits=[0 250]. mk 7jul2011
                  ...
                  % AKH 10/31/13 -- ^^ corrected it; baseline was converted
                  % from ms to samples twice.

%                 use mean V
                V(:,spoo)=squeeze(p.correctedV(findex, aindex, dindex, pindex, :));

                try deltaV(:,spoo) = V(:,spoo) - mean(V(1:baseline,spoo));
                catch
                    % Same issue as above ~line117
                    deltaV(:,spoo) = V(:,spoo) - mean(V(1:datalength,spoo));
                end
                %               %use raw V
...
%             fprintf('\nmean,max conf=%.2f, %.2f', mean(conf(:,2)-conf(:,1)),max(conf(:,2)-conf(:,1)))
            ...
            p.GE(findex, aindex, dindex, :)=ge;
            p.GI(findex, aindex, dindex, :)=gi;
            p.GSYN(findex, aindex, dindex, 1, :)=m;
            p.GSYN(findex, aindex,dindex, 2, :)=b;
            p.GSYNconf(findex, aindex, dindex, :,:)=conf;
        end
    end
end
function p=compute_bin_ge_gi(p, varargin)
%computes ge gi for binaural TC mw 033110
% usage:
% out=compute_bin_ge_gi(out);
% optional second argument for regress method
% out=compute_bin_ge_gi(out, 'regress');
% out=compute_bin_ge_gi(out, 'polyfit');
% inputs:
% out: structure generated by ProcessBinVCData
...
numamps=length(p.Ramps);
numpotentials=length(p.potentials);
numdurs=length(p.durs);

baseline=-p.xlimits(1)*p.samprate/1000; %baseline is in samples
if baseline<=0
    baseline=100; %baseline is used to extract synaptic currents from raw currents
end
        ...
        for Raindex=1:numamps
            for Laindex=1:numamps
                ...
                for pindex=1:numpotentials
                    is(:,pindex)=squeeze(p.mM1(findex,Raindex, Laindex, dindex, pindex,1, :));
                    vs(:,pindex)=squeeze(p.correctedV(findex,Raindex, Laindex, dindex, pindex, :));
                end
                ...
                %gl=mean(m);
                b=nanmean(b);
                p.EREST(findex, Raindex, Laindex)=er;
            end
        end
    end
end
er=mean(mean(nanmean(p.EREST))) ;
    ...
    for dindex=1:numdurs
        for Raindex=1:numamps
            for Laindex=1:numamps
                ...
                fprintf('\nf%d Ra%d La%d d%d', findex, Raindex, Laindex, dindex);
                for pindex=1:numpotentials
                    spoo= pindex;
                    ...
                    rawI(:,:,spoo)=squeeze(p.M1(findex, Raindex, Laindex, dindex, pindex,1:min_num_reps, :)); %use all currents
                    ...
                    rawdeltaI(:,:,spoo) = rawI(:,:,spoo) - repmat(mean(rawI(:,1:baseline,spoo), 2), 1, datalength);

                    %use mean V
                    V(:,spoo)=squeeze(p.correctedV(findex,  Raindex, Laindex, dindex, pindex, :));

                    deltaV(:,spoo) = V(:,spoo) - mean(V(1:baseline,spoo));
                    %               %use raw V
                ...
                p.GE(findex, Raindex, Laindex, dindex, :)=ge;
                p.GI(findex, Raindex, Laindex, dindex, :)=gi;
                p.GSYN(findex, Raindex, Laindex, dindex, 1, :)=m;
                p.GSYN(findex, Raindex, Laindex,dindex, 2, :)=b;
                p.GSYNconf(findex, Raindex, Laindex, dindex, :,:)=conf;
            end
        end
    end
end
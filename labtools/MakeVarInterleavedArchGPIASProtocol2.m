function MakeVarInterleavedArchGPIASProtocol2
% usage: MakeVarInterleavedArchGPIASProtocol2
%Create interleaved VarPPALaser Protocol and incorporate a previously
% created GPIAS protocol
%this is version 2 which interleaves a laser pulse on every other GPIAS
%(identical repetition of the GPIAS), the laser is on exactly during the
%gap + post-gap-interval. I.e. laser onset coincides with gap onset, laser
%offset coincides with onset of startle pulse. Works in either ISI mode or
%SOA mode (either soaflag).

%modified from MakeInterleavedArchGPIASProtocol to use our new
%VarPPALaser method, which allows variable laser pulse params for each trial.
%The motivation is to have the laser on exactly during the gap, which is a
%different duration each trial. But the method is extensible.
%mw 10.07.2015
%
%inputs: still  not sure what inputs we need here
%
%outputs:
%   creates a suitably named stimulus protocol in exper2.2\protocols\Arch Protocols
%
%
%example call:
%  MakeVarInterleavedArchGPIASProtocol

%from MakeInterleavedArchGPIASProtocol:
...
while tc_n+1<=length(tc.stimuli)
    ...
    if tc_n>length(tc.stimuli) %need to break again to avoid adding a spurious laser pulse at end
        break
    end
    ...
    stimuli(st_n).param.VarPPA=1;
    stimuli(st_n).param.VarPPAstart=tone.param.gapdelay-tone.param.gapdur; %for example.  laser onset coincides w/ gap onset
    switch tone.param.soaflag
        case 'isi'
    stimuli(st_n).param.VarPPApulsewidth=tone.param.gapdur+tone.param.soa; %for example. laser lasts up until onset of startle pulse
        case 'soa'
    stimuli(st_n).param.VarPPApulsewidth=tone.param.soa; %for example. laser lasts up until onset of startle pulse
    end
    stimuli(st_n).param.VarPPAnumpulses=1;
    stimuli(st_n).param.VarPPAisi=100;
    %     edur=edur+tone.param.duration+tone.param.next;
...
stimuli(1).param.name= sprintf('VarPPALaser GPIAS2, %s', tc.stimuli(1).param.name);
str='the laser is on exactly during the gap + post-gap-interval. I.e. laser onset coincides with gap onset, laser offset coincides with onset of startle pulse.';
stimuli(1).param.description=sprintf('VarPPALaser, GPIAS2 (%s) %s', str, tc.stimuli(1).param.description);
filename=sprintf('VarPPALaser GPIAS2-%s', tcfilename);

function MakeVarInterleavedASRProtocol1
% usage: MakeVarInterleavedASRProtocol1
%Create interleaved VarPPALaser Protocol and incorporate a previously
% created PPI protocol
%this is version 1 which interleaves a laser pulse on every other trial
%(identical repetition of the prepulse), the laser is on exactly during the
%prepulse. I.e. laser onset coincides with prepulse onset, laser duration
%matches prepulse duration
%
%mw 3.17.2017 ???
%
%inputs: none. Prompts with a dialog box to select PPI protocol.
%
%outputs:
%   creates a suitably named stimulus protocol in exper2.2\protocols\ASR Protocols
%
%
%example call:
%  MakeVarInterleavedASRProtocol1
...
[tcfilename, tcpathname] = uigetfile('*.mat', 'Choose PPI protocol to incorporate into interleaved protocol:');
...
while tc_n+1<=length(tc.stimuli)
    ...
    %then add the PPI with laser on
    stimuli(st_n)=tone;
    
    stimuli(st_n).param.AOPulseOn=1;
    stimuli(st_n).param.VarPPA=1;
    stimuli(st_n).param.VarPPAstart=0; %for example.  laser onset coincides w/ stimulus onset
    stimuli(st_n).param.VarPPApulsewidth=tone.param.prepulsedur; %for example
    stimuli(st_n).param.VarPPAnumpulses=1;
    stimuli(st_n).param.VarPPAisi=100; %not used
    ...
    %then add the same stimulus with laser  off
    ...
    %MakeGapShockProtocol if you want a fear conditioning protocol
...
stimuli(1).param.name= sprintf('VarPPALaser ASR1, %s', tc.stimuli(1).param.name);
str='laser is on exactly during the prepulse. I.e. laser onset coincides with prepulse onset, laser duration matches prepulse duration';
stimuli(1).param.description=sprintf('VarPPALaser, GPIAS1 (%s) %s', str, tc.stimuli(1).param.description);
filename=sprintf('VarPPALaser ASR1-%s', tcfilename);

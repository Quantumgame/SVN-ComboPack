function p=get_bin_correctedV(p)
%%correct potentials for series error
%usage: p=get_bin_correctedV_adapt(p)
...
for dindex=1:length(p.durs)
    for findex=1:length(p.freqs)
        for Raindex=1:length(p.Ramps)
            for Laindex=1:length(p.Lamps)
                for pindex=1:length(p.potentials)
                    mI(pindex,:)=squeeze(p.mM1(findex, Raindex, Laindex, dindex, pindex,1, :))';
                    ...
                    p.correctedV(findex, Raindex, Laindex, dindex, pindex, :)=p.corrected_potentials(pindex)-p.meanrs*mI(pindex,:)/1000;

%getN1_TTS_manual
% script to look at ABR TCs before and after a trauma, and manually pick
% out the threshold shift of the N1 component
...
H=-1*ones(numfreqs, numamps);
fig=figure;
p=0;
while length(find(H<0))

p=p+1;fprintf('\npass %d', p)
    for aindex=[numamps:-1:1]
        for findex=1:numfreqs
            if H(findex, aindex)<0
            ...
            title([a, f])
            set(fig, 'pos', [89   646   560   420])
            shg
            h=questdlg('is there an N1?', 'click red X to bail', 'Yes','No','Not sure','Yes');
            switch h
                case 'Yes'
                    H(findex, aindex)=1;
                case 'No'
...
Hname=sprintf('H%s-%s-%s',expdate,session, filenum);
save(Hname, 'H')
    return

function CheckPerformanceGap_Silent
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if winSize<min(length(lCorrect),length(rCorrect))
    ...
    bwin50(1:length(lCorrect)) = mean(lCorrect);sudo /usr/local/MATLAB/R2016b/bin/matlab
password: A1
...
if length(response)>3000
    for i = winSize:last
        ...
        endsudo /usr/local/MATLAB/R2016b/bin/matlab
password: A1
    end
else
    for i = winSize:last
...
dur_correct = [mean(correct(gapDur==0)),...
    ...
    mean(correct(gapDur==129))];

dur_correct(isnan(dur_correct)) = 0;
hold on
bar(dur_correct)
xticklabels({'0','1','2','4','8','16','32','64','128'})
hold off

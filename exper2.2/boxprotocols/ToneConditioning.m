function varargout=ToneConditioning(varargin)
...
case 'eboxmastertrialend'
    ...
    if length(trial)>1
        ...
        if ~isempty(toneStartIdx)
            ...
            rewardPeriod=(GetParam(me,'ToneDuration')/1000+GetParam(me,'CurrentISI'))*0.2;  % currentisi is in sec!!!
                                                        % let's look at
                                                        % performance in
                                                        % first 20 % of isi
            ...
            if ~isempty(timesIdx)
                ...
                if totalTrialLicks>0
                    SetParam(me,'Performance',correctTrialLicks/totalTrialLicks);
                else
                    SetParam(me,'Performance',0);
                end
...
% this creates all the ui controls for this module
	fig = ModuleFigure(me,'visible','off');	
        ...
        correct=0;
        InitParam(me,'Performance','ui','edit','value',correct,'pref',0,'units','normal','pos',[h 9*height+h 0.4 height]);
        SetParamUI(me,'Performance','label','Performance');
        error=0;
        InitParam(me,'TotalLicks','ui','edit','value',error,'pref',0,'units','normal','pos',[h 8*height+h 0.4 height]);
...
% reward=GetParam(me,'ToneDuration')/1000;  % reward period
...
isi=tonedur+random('unif',8,16);         % 'isi' interval (seconds)
SetParam(me,'CurrentISI',isi);

...
switch action
    ...
    case 'load'
        ...
        try
            ...
            Message(me,'Cannot load sound', 'append');
        end
...
Message(me, sprintf('Initialized PsychPortAudio with device %d, reqlatencyclass %d, Fs %d, numChan %d, buffersize %d\n\n', deviceid, reqlatencyclass, SoundFs, numChan, buffSize));
...
%laserON==1
if ExistParam('PPALaser', 'on')
    on=GetParam('PPALaser', 'on');
    if on
        if isfield(param, 'AOPulseOn')
            if param.AOPulseOn %then deliver a laser pulse
                str='Laser pulse *';
                start=GetParam('PPALaser', 'start'); %ms
                pulsewidth=GetParam('PPALaser', 'width'); %ms
                numpulses=GetParam('PPALaser', 'numpulses'); %ms
                isi=GetParam('PPALaser', 'isi'); %ms
                if length(pulsewidth)>1
                    if length(pulsewidth)~=numpulses;
                        message(me, 'PPALaser: numpulses must match number of widths', 'append')
                    end
                        if length(isi)~=numpulses-1;
                            message(me, 'PPALaser: numpulses must be 1 more than number of isis', 'append')
                        end
                end
                if length(pulsewidth)==1
...
%         Message(me, str, 'append');
        ...
        if status.Active==0; %device not running, need to start it
            ...
            fprintf(fid, '\nnrepeats %d', getparam('stimulusprotocol', 'nrepeats'));
            fclose(fid);
            fid=fopen(fullfile(getparam('control','fulldatapath'),[getparam('control', 'fullexpiddatafile'), 'seamless_restarts.txt']), 'a');

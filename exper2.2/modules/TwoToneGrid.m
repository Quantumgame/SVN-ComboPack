function out=TwoToneGrid(varargin)
...
function InitializeTraces
    ...
    try
        expr=sprintf('unique([param.%s]);',dparam);  % get the tone duration
        tonedur=eval(expr);
        expr=sprintf('unique([param.%s]);','SOA');  % get the SOA
        SOA=eval(expr);
    catch
        tonedur=100;
        SOA=80;
    end
    duration=tonedur+SOA; %add 50ms to see end of response

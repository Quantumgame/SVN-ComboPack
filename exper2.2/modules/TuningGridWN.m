function out=TuningGridWN(varargin)
...
case 'edataavailable'
    if GetParam(me,'Watch')
        ...
        if strcmpi(stimulus.type,type) | strcmpi(stimulus.type,'whitenoise')
            ...
            if find(strcmpi(stimulus.type,type)) %if tone, x=freq
                x=eval(expr);
            else x=-1;
            end
            xpos=find(xaxis==x);
            ...
            if find(strcmpi(stimulus.type,type)) %if tone, x=freq
                ...
                Message(me, sprintf('whitenoise, %ddB',  y));
            end
            % TEMPORARY
...
% 	SetParamUI(me,'Clear','string','Clear','label','');
    ...
    filelabelhandle=text(10, 5.65, sprintf('%s',getparam('control', 'fullexpiddatafile')));
    InitParam(me,'FileLabelHandle','value',filelabelhandle);    
...
function InitializeTraces
    ...
    xaxis=eval(expr);
    xaxis=[-1 xaxis]; %add -1 freq for white noise
...
%     set(tracehandles,'Parent',traceaxes);
    ...
    xL=get(traceaxes,'XLim');
    line([1.5 1.5], yL, 'color', 'r');
    text(.6, yL(2)+.03*diff(yL), 'WN')
    
       % filelabelhandle=text(mean(xL), yL(2)+.03*diff(yL), sprintf('%s',getparam('control', 'fullexpiddatafile')));
    filelabelhandle=GetParam(me,'FileLabelHandle'); 
    set(filelabelhandle, 'string', ...
        sprintf('%s',getparam('control', 'fullexpiddatafile')), ...
        'position', [mean(xL) yL(2)+.03*diff(yL) 0 ]);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
h=findobj(fig,'type','uicontrol','style','edit','tag', 'traceheight' );%mw 062306
set(h,'enable','on')
